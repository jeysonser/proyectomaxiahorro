        ...
        <nav class="navbar navbar-expand-lg navbar-dark" style="background-color: red">
                ...
                <div class="dropdown-menu nav-item bg-dark">  
                    <c:if test="${clientelogueado.id!=null&&clientelogueado.id!=0}">
                        ...
                        <c:if test="${clientelogueado.idrol==1}">
                            <a class="nav-link dropdown-item" style="color: red" href="./Controlador?accion=Productos"><i class="fa fa-angle-right"></i> Productos</a>                                                                         
                        </c:if>
                    </c:if>
                </div>
            </div>
            ...
            <div class="collapse navbar-collapse" id="navbarTogglerDemo03">
                <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
                    ...
                    <li class="nav-item active">
                        <a class="nav-link" href="./Controlador?accion=carrito"><i class="fas fa-cart-plus mr-1" style="color: white">(<label style="color:white">${cont}</label>)</i> Carrito</a>
                    </li>                               
                </ul>   
                <ul class="navbar-nav mr-auto mt-2 mt-lg-0">
                    <form action="Controlador" class="form-inline my-2 my-lg-0">
                        <input placeholder="Buscar" class="form-control " name="txtBuscar" style="border: 1px solid #ccc;" onfocus="this.style.borderColor = '#FFD700'" onblur="this.style.borderColor = '#ccc'">
                    ...
                    <div class="col-sm-4"><!--Solo va cambiar aqui puedes configurar de acuerdo al gusto-->
                        <div class="form-group">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <label class="btn btn-outline-dark"><i class="">S/. ${p.getPrecio()}</i></label>
                                ...
                                <div class="text-center">                             
                                    <img src="${p.getImagen()}" style="height: 230px;width:80%; padding: 10px">
                                </div>
                                <br>
                                <div class="card-footer">
                                    ...
                                    <div class=" col-sm-13 text-center">                                         
                                        ...
                                        <c:if test="${p.getStock()==0}"> 
                                            <button data-toggle="modal" data-target="#modalStock" class="btn btn-warning">Agregar a Carrito<i class="fas fa-cart-plus"></i></button>
                                            <button data-toggle="modal" data-target="#modalStock" class="btn btn-danger">Comprar</button>
                                        </c:if>
                                        <c:if test="${p.getStock()!=0}"> 
                                            <c:if test="${clientelogueado.id==null||clientelogueado.id==0}"> 
                                                <button data-toggle="modal" onclick="CargarModalLogueo1()" data-target="#myModalLogueo1" class="btn btn2 btn-warning">Agregar a Carrito<i class="fas fa-cart-plus"></i></button>
                                                <button data-toggle="modal" onclick="CargarModalLogueo1()" data-target="#myModalLogueo1" class="btn btn-danger">Comprar</button>
                                            </c:if>
                                            <c:if test="${clientelogueado.id!=null&&clientelogueado.id!=0}">
                                                <a href="Controlador?accion=AgregarCarrito&id=${p.getId()}" class="btn btn2 btn-warning">Agregar a Carrito<i class="fas fa-cart-plus"></i></a>
